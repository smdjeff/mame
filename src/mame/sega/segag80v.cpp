...
// The XY board's 8x8 hardware multiplier is an Am25LS14A (an 8-bit
// serial/parallel two's-complement multiplier -- confirmed via its datasheet
// and the X-Y Control Board schematic, sheet 6/6, U43). It asserts /WAIT on
// the backplane (pin 14, "\wait (used by Hw multiplier)") while clocking the
// operand in serially, stalling the Z80 until the product is ready -- see
// multiply_w()'s own comment. Per the Am25LS14A datasheet: an m-bit x n-bit
// multiply takes m+n clock cycles (8+8=16 here, the extra 8 being the Y
// operand's required sign-extension cycles to complete the two's-complement
// product) -- not a guess, this is the datasheet's own stated cycle count.
// The schematic's "MULTIPLY CLOCK (17)" net (built from a 74LS161 counter
// gating the chip's clock input, consistent with a self-terminating 16-pulse
// burst) runs off the board's 7.5MHz line -- close enough to VCL_CLOCK
// (VECTOR_CLOCK/2 = 7.73424MHz, segag80v_v.cpp) that it's almost certainly
...
void segag80v_state::machine_start()
{
	...
	save_item(NAME(m_vg_curx));
	save_item(NAME(m_vg_cury));
	save_item(NAME(m_vg_vecaddr));
	save_item(NAME(m_vg_header_snap));
	save_item(NAME(m_vg_vector_snap));
	save_item(NAME(m_vg_draw));
	save_item(NAME(m_vg_symangle));
	save_item(NAME(m_vg_scale));
	save_item(NAME(m_vg_attrib));
	save_item(NAME(m_vg_length));
	save_item(NAME(m_vg_vecangle));
	save_item(NAME(m_vg_deltax));
	save_item(NAME(m_vg_deltay));
	save_item(NAME(m_vg_draw_remaining));
	save_item(NAME(m_vg_xaccum));
	save_item(NAME(m_vg_yaccum));
	save_item(NAME(m_vg_clipped));
	save_item(NAME(m_vg_color));
	save_item(NAME(m_vg_intensity));
	save_item(NAME(m_vg_symbol_is_last));
	save_item(NAME(m_vg_idle));
}
...
void segag80v_state::multiply_w(offs_t offset, u8 data)
{
	...
	if (offset == 1)
	{
		// The Am25LS14A datasheet lists two distinct capabilities: "two's
		// complement multiplication without correction" AND, separately,
		// "magnitude only multiplication". This board's software (sega-vector's
		// math.c: div_16's reciprocal-table multiply, vectorToXY's sin/cos
		// scaling) relies on the LATTER -- it always tracks sign itself in
		// software (separate sin_neg/cos_neg-style flags) and feeds this port
		// raw magnitudes up to 255, e.g. vectorToXY's sin_table[] genuinely
		// reaches 255. Treating those bytes as signed here (an earlier version
		// of this function did) double-negates any magnitude >=128 on top of
		// software's own sign tracking, corrupting direction -- confirmed by a
		// visible regression (an on-screen logo animating the wrong way) once
		// tried. So: plain unsigned multiply, matching this board's actual
		// wiring/usage, not the signed mode the chip also happens to support.
		m_mult_result = m_mult_data[0] * m_mult_data[1];

		// second operand's write is what starts the chip clocking the multiply
		// in -- see MULTIPLY_WAIT_STATES' own comment for the estimate. (The
		...
		// correction above.) Skipped under the FPGA timing model: the real
		// FPGA has no hardware multiplier peripheral at these ports at all
		// (see m_fpga_timing's own comment, segag80v.h), so modeling this
		// stall here would charge a cycle count the real FPGA hardware
...
u8 segag80v_state::vector_pc_lsb_r()
{
	return m_vector_pc & 0xff;
}


u8 segag80v_state::vector_pc_msb_r()
{
...
{
	// exact now, not an approximation: the vector generator state machine (segag80v_v.cpp)
	// is genuinely busy walking the symbol table until it hits Last Symbol and goes idle.
	return !m_vg_idle;
}
...
void segag80v_state::vblank_callback(screen_device &screen, bool state)
{
	if (!state)
	{
		...
		// this IS the vector PC's 40Hz reset (see update_int()'s EDGINT comment below --
		// same divider chain off the same crystal): per the G80 hardware reference, "there
		// is no way to stop the vector PC from resetting every 40th of a second", so this
		// unconditionally restarts the walk from symbol 0, cutting off whatever the state
		// machine hadn't reached yet if it was still busy (or just waking it from idle).
		vg_start_pass();
...
void segag80v_state::main_portmap(address_map &map)
{
	...
	map(0xbc, 0xbc).r(FUNC(segag80v_state::vector_pc_lsb_r));
	map(0xbd, 0xbd).r(FUNC(segag80v_state::vector_pc_msb_r));
...
static INPUT_PORTS_START( startrek )
	...
	PORT_START("SPINNER")
	PORT_BIT( 0xff, 0x00, IPT_DIAL ) PORT_SENSITIVITY(100) PORT_KEYDELTA(1) PORT_RESET
INPUT_PORTS_END
